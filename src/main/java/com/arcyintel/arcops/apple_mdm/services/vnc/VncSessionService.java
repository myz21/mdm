...
/**
 * Manages VNC remote desktop sessions in Redis.
 */
@Service
@RequiredArgsConstructor
public class VncSessionService {

    private static final Logger logger = LoggerFactory.getLogger(VncSessionService.class);
    private static final String KEY_PREFIX = "vnc:session:";
    private static final String DEVICE_SESSION_PREFIX = "vnc:device:";
    private static final long INITIAL_TTL_MINUTES = 5;
    private static final long MAX_TTL_MINUTES = 120;

    private final RedisTemplate<String, Object> redisTemplate;

    public VncSession createSession(String deviceUdid) {
        String sessionId = UUID.randomUUID().toString();
        String now = Instant.now().toString();
        VncSession session = new VncSession(
                sessionId, deviceUdid, VncSession.STATE_PENDING, now, now
        );
        ...
        logger.info("VNC session created — sessionId={}, device={}", sessionId, deviceUdid);
        return session;
    }
    ...
    public VncSession getSession(String sessionId) {
        ...
        if (raw instanceof VncSession s) {
            return s;
        }
        return null;
    }
    ...
    public void updateState(String sessionId, String newState) {
        VncSession session = getSession(sessionId);
        if (session == null) {
            logger.warn("Cannot update state — VNC session not found: {}", sessionId);
            return;
        }
        VncSession updated = session.withState(newState);
        String key = KEY_PREFIX + sessionId;
        redisTemplate.opsForValue().set(key, updated, MAX_TTL_MINUTES, TimeUnit.MINUTES);
        logger.info("VNC session state updated — sessionId={}, state={}", sessionId, newState);
    }
    ...
    public void endSession(String sessionId) {
        ...
        logger.info("VNC session ended — sessionId={}", sessionId);
    }
}
